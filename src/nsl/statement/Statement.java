...
public abstract class Statement {
	...
	protected static ArrayList<Statement> globalUninstallerAssignmentStatements =
	...
	public static void addGlobal(Statement add) {
		if (Scope.inUninstaller()) globalUninstallerAssignmentStatements.add(add);
		else globalAssignmentStatements.add(add);
	}
	 ...
	 * @return the instruction or <code>null</code> if the current token does not match an NSIS
	 *     instruction
	 */
	public static AssembleExpression matchInstruction(int returns) {
		if (ScriptParser.tokenizer.match(AbortInstruction.name)) return new AbortInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(BGFontInstruction.name)) return new BGFontInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(CreateShortCutInstruction.name)
				|| ScriptParser.tokenizer.match("CreateShortcut"))
			return new CreateShortCutInstruction(returns);
		if (ScriptParser.tokenizer.match(DeleteInstruction.name)) return new DeleteInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(DirVarInstruction.name)) return new DirVarInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(FileInstruction.name)) return new FileInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(IfShellVarContextAllInstruction.name))
			return new IfShellVarContextAllInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(LogSetInstruction.name)) return new LogSetInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(NopInstruction.name)) return new NopInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(QuitInstruction.name)) return new QuitInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(RebootInstruction.name)) return new RebootInstruction(returns);
		if (ScriptParser.tokenizer.match(RegDLLInstruction.name)) return new RegDLLInstruction(returns);
		if (ScriptParser.tokenizer.match(RenameInstruction.name)) return new RenameInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(RMDirInstruction.name)) return new RMDirInstruction(returns);
		...
		if (ScriptParser.tokenizer.match(StrLenInstruction.name)) return new StrLenInstruction(returns);
		if (ScriptParser.tokenizer.match(SubCaptionInstruction.name))
			return new SubCaptionInstruction(returns);
		if (ScriptParser.tokenizer.match(TargetInstruction.name)) return new TargetInstruction(returns);
	...
	public static Statement match() {
		...
		if (statement != null
				&& (statement instanceof AssignmentStatement || statement instanceof FunctionCallStatement)
				&& !FunctionInfo.in()
				&& !SectionInfo.in()) {
			if (statement instanceof FunctionCallStatement) {
				if (((FunctionCallStatement) statement).getExpression() instanceof FunctionCallExpression)
					throw new NslContextException(
							EnumSet.of(NslContext.Section, NslContext.Function), "function call");
				if (((FunctionCallStatement) statement).getExpression() instanceof PluginCallExpression)
					throw new NslContextException(
							EnumSet.of(NslContext.Section, NslContext.Function), "plug-in call");
			}
			...
			if (!(statement instanceof AssignmentStatement)
					|| !ExpressionType.isString(((AssignmentStatement) statement).getExpression())
					|| !((AssignmentStatement) statement).getExpression().getStringValue().isEmpty())
				addGlobal(statement);
			return match();
		}
	...
	private static Statement matchInternal() {
		if (ScriptParser.tokenizer.tokenIsWord()) {
			...
			if (IfDirective.in()) {
				if (ScriptParser.tokenizer.tokenIs("#else")
						|| ScriptParser.tokenizer.tokenIs("#elseif")
						|| ScriptParser.tokenizer.tokenIs("#endif")) return null;
			}
			...
			if (ScriptParser.tokenizer.sval.startsWith("$")) return new AssignmentStatement();

			// Match a pre-processor directive.
			if (ScriptParser.tokenizer.match("#define")) return new DefineDirective();
			if (ScriptParser.tokenizer.match("#redefine")) return new RedefineDirective();
			if (ScriptParser.tokenizer.match("#if")) return new IfDirective();
			if (ScriptParser.tokenizer.match("#macro")) return new MacroDirective();
			if (ScriptParser.tokenizer.match("#include")) return new IncludeDirective();
			if (ScriptParser.tokenizer.match("#undef")) return new UndefDirective();
			if (ScriptParser.tokenizer.match("#error")) return new ErrorDirective();

			// Important that we don't use match() here otherwise it eats into the
			// NSIS code.
			if (ScriptParser.tokenizer.tokenIs("#nsis")) return new NSISDirective();
			...
			if (ScriptParser.tokenizer.match("uninstall")) {
				...
				} else if (ScriptParser.tokenizer.match("function")) statement = new FunctionStatement();
				else if (ScriptParser.tokenizer.match("section")) statement = new SectionStatement();
				else if (ScriptParser.tokenizer.match("sectiongroup"))
					statement = new SectionGroupStatement();
				else if (ScriptParser.tokenizer.match("page")) statement = new PageStatement();
			...
			if (ScriptParser.tokenizer.match("function")) return new FunctionStatement();
			if (ScriptParser.tokenizer.match("section")) return new SectionStatement();
			if (ScriptParser.tokenizer.match("sectiongroup")) return new SectionGroupStatement();
			if (ScriptParser.tokenizer.match("page")) return new PageStatement();

			if (ScriptParser.tokenizer.match("if")) return new IfStatement();
			if (ScriptParser.tokenizer.match("while")) return new WhileStatement();
			if (ScriptParser.tokenizer.match("do")) return new DoStatement();
			if (ScriptParser.tokenizer.match("for")) return new ForStatement();
			if (ScriptParser.tokenizer.match("return")) return new ReturnStatement();
			if (ScriptParser.tokenizer.match("break")) return new BreakStatement();
			if (ScriptParser.tokenizer.match("continue")) return new ContinueStatement();
			if (ScriptParser.tokenizer.match("switch")) return new SwitchStatement();
			...
			if (!(expression instanceof AssembleExpression))
				throw new NslException(
						"\"" + expression.toString(true) + "\" is not a valid statement", true);
			...
			if (expression instanceof AssignmentExpression) return new AssignmentStatement(expression);

			// Wrap the function or plug-in call inside a function call statement.
			if (expression instanceof FunctionCallExpression
					|| expression instanceof PluginCallExpression)
		...
		if (ScriptParser.tokenizer.tokenIsChar()) {
			// EOF found.
			if (ScriptParser.tokenizer.ttype == Tokenizer.TT_EOF) return null;

			// Function call with multiple return values.
			if (ScriptParser.tokenizer.tokenIs('(')) return new FunctionCallStatement();

			// New block of code.
			if (ScriptParser.tokenizer.tokenIs('{')) return new BlockStatement();

			// Skip ;
			if (ScriptParser.tokenizer.match(';')) return match();

			// Odd character?
			if (!ScriptParser.tokenizer.tokenIs('}')) throw new NslExpectedException("a statement");
		}
