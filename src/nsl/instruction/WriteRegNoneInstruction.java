 ...
 * @author Jan
 */
public class WriteRegNoneInstruction extends AssembleExpression {
	public static final String name = "WriteRegNone";
	 ...
	 */
	public WriteRegNoneInstruction(int returns) {
		if (!SectionInfo.in() && !FunctionInfo.in())
			throw new NslContextException(EnumSet.of(NslContext.Section, NslContext.Function), name);
		if (returns > 0) throw new NslReturnValueException(name);

		ArrayList<Expression> paramsList = Expression.matchList();
		if (paramsList.size() != 3 && paramsList.size() != 4)
			throw new NslArgumentException(name, 3, 4);
		...
		this.value = paramsList.size() == 4 ? paramsList.get(3) : null;
	}
	...
	public void assemble() throws IOException {
		...
		if (this.value != null) {
			...
			ScriptParser.writeLine(
					name + " " + this.rootKey + " " + varOrSubKey + " " + varOrValueName + " " + varOrValue);
