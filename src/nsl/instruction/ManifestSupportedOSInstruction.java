...
public class ManifestSupportedOSInstruction extends AssembleExpression {
	...
	private static final Set<String> VALID_OS_VALUES =
			new HashSet<>(
					Arrays.asList(
							/**
	...
	public ManifestSupportedOSInstruction(int returns) {
		...
		if (this.values.isEmpty()) throw new NslArgumentException(name, 1, Integer.MAX_VALUE);
			...
			if (!VALID_OS_VALUES.contains(osValue)) {
				throw new NslException(
						String.format(
								...
								name, osValue, i + 1, String.join(", ", VALID_OS_VALUES)),
						true);
			}
		}
	}
